...
public class PosterChangeFaceActivity extends AppCompatActivity implements PosterPhotoRenderer.OnRendererStatusListener,
        ...
        if (mIsNeedReInput) {
            ...
            if (!mIsNotSelectedMultiFace && !mIsFirstDraw) {
                ...
                while (mIsTrackedTemplate && retry && ++mRetryCount <= 10) {
                    ...
                    Log.e(TAG, "onDrawFrame: retry " + retry + ", count:" + mRetryCount);
                }
                if (texId == 0) {
                    mIsNeedReInput = true;
                    mState = STATE_FAILED;
    ...
    public void onTemplateLoaded(byte[] img, int width, int height) {
        Log.e(TAG, "onTemplateLoaded() called width = [" + width + "], height = [" + height + "]");
    ...
    public void onPhotoLoaded(byte[] img, int width, int height) {
        Log.e(TAG, "onPhotoLoaded() called width = [" + width + "], height = [" + height + "]");
        // 多人脸弹框过程中
        ...
        if (trackFace > 0) {
            if (trackFace > 1) {
                ...
                if (!checked) {
                    ...
                    mIsNeedReInput = true;
                    mPhotoLandmarks = getCopyOfLandmark(mFaceIndex);
                    Log.e(TAG, "onPhotoLoaded: landmarks " + Arrays.toString(mPhotoLandmarks));
                    mPosterPhotoRenderer.reloadTemplateData(mTemplatePath);
                }
            }
        } else {
